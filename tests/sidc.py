...
class Test_Airdrop():
    ...
    def Participate_Airdrop(self):
        ...
        for address in eth_ads:
            #处理提交数据请求
            count = count+1
            Date=t.strftime(fmt,t.localtime(t.time()))     #把传入的元组按照格式，输出字符串
            print(Date+ ":current address->"+address+"__NO is->"+str(count)+" _submit is start ....")
            dr.find_element_by_xpath("//*[@type='text']").clear()
            t.sleep(random.randrange(20,50))#随机20-50秒之间
            ...
            t.sleep(random.randrange(5, 9, 2))#随机
            print("current address->"+address+"__NO is->"+str(count)+" _submit is  end.")
